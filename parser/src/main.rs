...
/// .0 から .1 までの区間を表す位置情報.
/// `Loc(4. 6)` なら入力文字の 5 文字目から 7 文字目までの区間を表す.
...
/// `pos` のバイトが期待するものであれば 1 バイト消費して `pos` を 1 進める.
...
/// AST (抽象構文木, Abstract Syntax Tree) を表すデータ型.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
enum AstKind {
    /// 数値
    Num(u64),
...
/// 左再帰性を除去して `EXPR3 = EXPR2 EXPR3_Loop`, `EXPR3_Loop = ("+" | "-") EXPR2 EXPR3_Loop | ε`
...
// `EXPR2 = EXPR2, ("*" | "/"), EXPR1 | EXPR1` から左再帰性を除去する.
...
// `Error` データ型と名前が重複するので `StdError` として導入
        ...
        let (e, loc): (&dyn StdError, Loc) = match self {
            ...
            Parser(e) => {
                let loc = match e {
                    ...
                    // `redundant expression` はトークン以降行末までが余りなので `loc` の終了位置を調整する
                    P::RedundantExpression(Token { loc, .. }) => Loc(loc.0, input.len()),
                    // `Eof` は `loc` 情報をもっていないのでその場で作る
